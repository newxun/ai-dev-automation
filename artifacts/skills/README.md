...
- `skill-bug-record/`：手动调用，在执行完某个自定义 skill 后，把发现的、关于该 skill 定义本身的问题（不是业务代码 bug）追加记录到跨仓库共享的日志文件。
- `skill-bug-triage/`：手动调用，集中处理 `skill-bug-record` 积累的待处理记录，逐条判断要不要修改对应 skill 的定义文件并回写处理状态。
