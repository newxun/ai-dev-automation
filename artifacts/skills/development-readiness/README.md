...
- 任务附带设计稿（如蓝湖/Figma 链接），需要区分「UI 呈现以设计稿为准、行为逻辑以需求文档为准」，并在设计稿与需求文档/原型稿冲突时被提醒，而不是被默默按需求文档实现；没有设计稿的 UI 任务也需要知道此时原型稿的布局信息可以被采信。
- 需求文档本身可信度存疑（例如由 AI 撰写未经校对，或改动未同步）时，建议先用 [`requirement-consistency-check`](../requirement-consistency-check/README.md) 核对需求文档自身一致性、以及需求文档与原型图/设计稿的 gap，再把核对通过的需求文档作为本 Skill 的权威材料来源；两者是独立 Skill，不自动联动。
...
- UI 任务按 UI 呈现/行为逻辑分维度采信设计稿与需求文档；有设计稿时原型稿只用于理解交互流程，没有设计稿时原型稿的布局信息可以被采信；发现明显出入直接暴露，不静默选择、不为此发起全量设计稿比对。
- 有设计稿时默认只记录链接/稿件标识，不主动调用设计稿类工具获取数据；确实获取到数据或用户已提供设计稿本体时，原样落盘或引用其实际目录，任务材料正文不放 AI 转译的自然语言描述。
